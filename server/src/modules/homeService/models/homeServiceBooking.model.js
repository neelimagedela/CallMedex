...
const createHomeServiceBooking = async (bookingData) => {
  ...
  const [result] = await db.execute(
    `INSERT INTO home_service_bookings (
      public_booking_id, user_id, assigned_phlebo_id,
      patient_name, patient_age, patient_sex,
      patient_mobile, patient_email, patient_address,
      patient_lat, patient_lng, branch, tests,
      collection_date, time_slot, prescription_path,
      total_amount, status
    ) VALUES (?, ?, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      publicBookingId, bookingData.userId,
      bookingData.patientName, bookingData.patientAge, bookingData.patientSex,
      bookingData.patientMobile, bookingData.patientEmail, bookingData.patientAddress,
      bookingData.patientLat || null, bookingData.patientLng || null,
      bookingData.branch, testsJson, bookingData.collectionDate,
      bookingData.timeSlot, bookingData.prescriptionPath || null,
      bookingData.totalAmount, bookingData.status || "pending",
    ]
  );

  return { id: result.insertId, publicBookingId, ...bookingData };
};

const getPhleboProfileByUserId = async (phleboUserId) => {
  const [rows] = await db.execute(
    `SELECT
      u.id AS user_id, u.public_user_id, u.name, u.email, u.phone, u.role,
      pp.id AS profile_id, pp.phlebo_type, pp.qualification, pp.specialization,
      pp.years_of_experience, pp.certification_number, pp.available_days,
      pp.available_time, pp.morning_start, pp.morning_end,
      pp.evening_start, pp.evening_end, pp.home_collection,
      pp.emergency_availability, pp.government_id_type,
      pp.aadhaar_front, pp.phlebotomy_certificate,
      pp.created_at, pp.updated_at
     FROM users u
     LEFT JOIN phlebo_profiles pp ON pp.user_id = u.id
     WHERE u.id = ? AND u.role = 'phlebo'
     LIMIT 1`,
    [phleboUserId]
  );
  return rows[0] || null;
...
const upsertPhleboLiveLocation = async ({ phleboUserId, latitude, longitude, isAvailable = true }) => {
  await db.execute(
    `INSERT INTO phlebo_live_locations (phlebo_user_id, latitude, longitude, is_available)
     VALUES (?, ?, ?, ?)
  ...
  return { phleboUserId, latitude, longitude, isAvailable };
};

const getPhleboLiveLocation = async (phleboUserId) => {
  const [rows] = await db.execute(
    `SELECT * FROM phlebo_live_locations
     WHERE phlebo_user_id = ? AND is_available = TRUE LIMIT 1`,
    [phleboUserId]
  );
  return rows[0] || null;
};

const getNearbyPendingBookingsForPhlebo = async (phleboUserId, radiusKm = 10) => {
  ...
  const [rows] = await db.execute(
    `SELECT hsb.*,
      (6371 * ACOS(
        COS(RADIANS(?)) * COS(RADIANS(hsb.patient_lat)) *
        COS(RADIANS(hsb.patient_lng) - RADIANS(?)) +
        SIN(RADIANS(?)) * SIN(RADIANS(hsb.patient_lat))
      )) AS distance_km
...
const resubmitRejectedBooking = async (bookingId, phleboUserId) => {
  const [result] = await db.execute(
    `UPDATE home_service_bookings
     SET status = 'submitted_to_lab', updated_at = CURRENT_TIMESTAMP
     WHERE id = ? AND assigned_phlebo_id = ? AND status = 'sample_rejected'`,
...
const acceptHomeServiceBooking = async (bookingId, phleboUserId) => {
  const [result] = await db.execute(
    `UPDATE home_service_bookings
     SET assigned_phlebo_id = ?, status = 'accepted', accepted_at = NOW()
     WHERE id = ? AND assigned_phlebo_id IS NULL AND status = 'pending'`,
...
const getPhleboActiveBooking = async (phleboUserId) => {
  const [rows] = await db.execute(
    `SELECT * FROM home_service_bookings
     WHERE assigned_phlebo_id = ?
       AND status IN ('accepted', 'sample_collected')
