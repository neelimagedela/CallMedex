...
export const updateScanStatus = async (id, currentStatus) => {
  const response = await api.patch("/api/staff/lab-technician/scan-status", {
    id,
    currentStatus,
  });
...
export const updateHomeServiceStatus = async (
  id,
  currentStatus,
  action = "accept"
) => {
  const response = await api.patch(
    "/api/staff/lab-technician/home-service-status",
    {
      id,
      currentStatus,
      action,
