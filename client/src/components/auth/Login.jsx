  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Short delay so toast is visible before page switches
        setTimeout(() => setPage("home"), 800);

      } else if (response.data.requiresVerification) {
        setUserId(response.data.data.userId);
        setShowOtpScreen(true);
        toast.info("Your email is not verified yet. Please enter the OTP sent to your email.");
      ...
      if (err.response?.status === 401) {
        ...
        toast.warning("Too many login attempts. Please wait a few minutes before trying again.");
      } else {
        toast.error(msg || "Unable to connect to server. Please check your internet and try again.");
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleOtpSubmit = async (e) => {
    ...
    try {
      ...
      if (status === 400) {
        ...
        toast.error(msg || "Something went wrong during verification. Please try again.");
      }
    } finally {
      setLoading(false);
    }
